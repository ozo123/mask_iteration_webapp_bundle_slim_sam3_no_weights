...
def edt_triton(data: torch.Tensor):
    ...
    if triton is None:
        ...
        device = data.device
        data_cpu = data.detach().to(torch.uint8).cpu().numpy()
        outputs = []
        for mask in data_cpu:
            mask_uint8 = mask.astype(np.uint8, copy=False)
            outputs.append(cv2.distanceTransform(mask_uint8, cv2.DIST_L2, 0))
        output = np.stack(outputs, axis=0).astype(np.float32, copy=False)
